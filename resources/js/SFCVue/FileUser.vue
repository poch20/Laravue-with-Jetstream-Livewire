        ...
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-body table-responsive p-0">
              ...
              @submit.prevent="fsp__formTableData(AuthUserId)">
...
export default {
  ...
  methods: {
    apiRequestPHPCallBySessionId(AuthUserSessionId){
      axios.get(`api/custom-get-keyResumeDataContr-by/${AuthUserSessionId}`).then( ({data}) => (this.ResumeDataModel = data))
    },
    ...
    getResumeAPIResponseData(id){
      ...
      axios.get(`api/get-resume-data-returns/${id}`)
           .then((response)=>{
             ...
             CustomEventHandler.emit('If the last checkbox was untick', this.apiRequestPHPCallBySessionId(AuthUserSessionId));
           })
    },
    ...
    fsp__formTableData(AuthUserSessionId){
      ...
      axios.post(`api/post-res-vue-del/${this.vmAttr__InpTChildrenColCheckbox}`,
        ...
        CustomEventHandler.emit('AfterDeleting', function(){
          self.apiRequestPHPCallBySessionId(AuthUserSessionId)
  ...
  created() {
    console.log(`ID PROPS ${this.AuthUserId}`)
    if(this.AuthUserId){
      let AuthUserSessionId = this.AuthUserId
      this.apiRequestPHPCallBySessionId(AuthUserSessionId)


      CustomEventHandler.on('Search Implementation On User Uploads', (goUpTheScopeChainToFindClosure) => {
        ...
        axios.get('api/search-user-uploads-of-type-user?anyVar=' + query)
        .then( (data) => {this.ResumeDataModel = data.data} )
        .catch( () => {})
